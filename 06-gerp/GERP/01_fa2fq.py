...
# Author: Tom van der Falk

# python3 fa2fq.py INPUTFILE OUTPUTFILE
...
def make_fastq(fasta_file,outputfile):
    ...
    header = 0
    "loop through fasta dictionary in windows of 50bp"
    for key,value in fasta_dict.items():
        for i in range(0,len(value),50):
            fastq_read = str(value[i:i+50])
            "make sure no missing bases are in the sequence and length is correct"
            if fastq_read.count("N") == 0 and len(fastq_read) == 50:
                "assign header"
                header += 1
                if header % 100000 == 0:
                    print(header)
